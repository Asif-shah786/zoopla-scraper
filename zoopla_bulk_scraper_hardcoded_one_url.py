...
async def fetch_property_details(property_data: dict):
    ...
    try:
        ...
        if lat and lng:
            ...
            print(f"   📍 Added coordinates to output: lat={lat}, lng={lng}")

        print(f"✅ Extracted complete details for property {property_id}")

        # Debug: Check if we got stations/schools data and determine source
        if "nearest_stations" in final_data:
            stations_data = final_data["nearest_stations"]
            if (
                isinstance(stations_data, str)
                and "miles" in stations_data
                and any(
                    x in stations_data for x in ["national_rail_station", "tube", "bus"]
                )
            ):
                print(
                    f"   📍 Found {len(stations_data.split(','))} stations via GraphQL API"
                )
            else:
                print(
                    f"   📍 Found {len(stations_data.split(','))} stations via HTML fallback"
                )
        if "nearest_schools" in final_data:
            schools_data = final_data["nearest_schools"]
            if (
                isinstance(schools_data, str)
                and "miles" in schools_data
                and any(
                    x in schools_data
                    for x in ["Good", "Outstanding", "Requires improvement"]
                )
            ):
                print(
                    f"   🏫 Found {len(schools_data.split(','))} schools via GraphQL API"
                )
            else:
                print(f"   🏫 Found schools data via HTML fallback")
